...
\begin{proof}[Beweis von Satz \ref{hurewicz-mod-c}]
  ...
  \[ E^2_{p,q} \cong H_p(X_{n-1}; \underbrace{H_q(K(\pi_n(X), n); \Z)}_{= 0 \text{ für } 0 < q < n}), \]
...
\begin{satz}[Relatives Mod-$\SC$-Hurewicz-Theorem]
  Es sei $\SC \in \{ \FG, \F \}$, $(X, A)$ ein Raumpaar, wobei $A$ nichtleer und einfach zusammenhängend ist.
...
\begin{proof}
  ...
  \begin{center}
    ...
    \end{tikzcd}
  \end{center}
  Aufgrund der absoluten Version des Theorems (Satz~\ref{hurewicz-mod-c}) ist also der linke Pfeil im folgenden Diagramm ein Isomorphismus mod $\SC$ für $i \leq n$.
  Gleiches gilt somit für den rechten Pfeil.
\end{proof}

\begin{kor}
  Es sei $f : A \to B$ stetig und $A$ einfach zusammenhängend.
  Angenommen, $f_* : \pi_i(A) \to \pi_i(B)$ ist ein Isomorphismus mod~$\SC$ für $1 \leq i < n$ und ein Epimorphismus mod~$\SC$ für $i = n$.
  Dann ist auch $f_* : H_i(A) \to H_i(B)$ ein Isomorphismus mod~$\SC$ für $1 \leq i < n$ und ein Epimorphismus mod~$\SC$ für $i = n$.
\end{kor}

\begin{proof}
  Betrachte den Abbildungszylinder $M_f$ von $f$.
  Die Abbildung $f$ entspricht der Inklusion $A \hookrightarrow M_f$ von $A$ als Deckel des Abbildungzylinder, wenn man $X$ als Deformationsretrakt als von $M_f$ auffasst.
  Aus der langen exakten Sequenz von Homotopiegruppen folgt, dass $\pi_i(M_f, A) \in \SC$ für $i \leq n$.
  Nach dem letzten Satz gilt somit $H_i(M_f, A) \in \SC$ für $1 \leq i \leq n$.
  Die Aussage folgt nun mit der langen exakten Sequenz von Homologiegruppen.
\end{proof}

% TODO: Effekt von Tensorieren mit $\Z_{(p)}$ beschreiben
Sei $G$ eine abelsche Gruppe, $p \in \N$ eine Primzahl.
...
\end{itemize}
Sei $X$ nun ein topologischer Raum.
...
Wir können somit die $p$-Torsion in (ganzzahliger) Homologie und Kohomologie durch bestimmen, indem wir die Homologie- und Kohomologiegruppen mit Koeffizienten mit $\Z_{(p)}$ berechnen.

% 1.29 in Hatcher SS1
\begin{lem}
  Es sei $n \geq 3$ und $p$ prim.
  Dann gilt $\Z_{(p)} \otimes H^i(K(\Z, n); \Z) = 0$ für $i < 2p + n - 1$ und $\Z_{(p)} \otimes H^i(K(\Z, n); \Z) \cong \Z_{(p)}$.
\end{lem}

\begin{proof}
  Wir verwenden wieder die Faserung $K(\Z, n-1) \to K(\Z, n)$ und die dazugehörige Serre-Spektralsequenz, diesmal mit Koeffizienten in $\Z_{(p)}$.
  \emph{Falls $n = 3$:} \enspace
  Wir können die Faser, einen $K(\Z, 2)$-Raum, als unendlich-dimensionalen komplexen projektiven Raum realisieren.\footnote{Sei $S^\infty \subset \C^\infty$ die unendlich-dimensionale Sphäre mit der Äquivalenzrelation $a \sim b :\!\iff \exists \, \lambda \in S^1 \subset \C \,:\, a = \lambda \cdot b$. Setze $\C P^\infty \coloneqq S^\infty / {\sim}$. Anhand der langen exakten Sequenz zur Faserung $S^1 \to S^\infty \to \C P^\infty$ erkennt man, dass $\C P^\infty$ ein $K(\Z, 2)$ ist.}
  ...
  Daher gilt $H^{2i}(\C P^\infty) = \Z$ und $H^{2i+1}(\C P^\infty) = 0$.
...
% TODO:
% * Namen für Axiome der Serre-Klassen und K(G, n) \in C für G \in C zu eigenem Axiom machen
% Statt den Satz nur für \FG, \F zu zeigen, als Voraussetzung nehmen, dass die (reduzierte) Homologie von K(G, n) für G \in C wieder in C liegt
