...
Das zelluläre Approximationstheorem \cite[\mbox{}4.8]{hatcher:at} besagt, dass jede Abbildung $f : S^i \to S^n$ homotop relativ Basispunkt zu einer zellulären Abbildung ist, \dh{} einer Abbildung $\tilde{f} : S^i \to S^n$, die das $i$-Skelett von $S^i$ (das ist ganz $S^i$) auf das $i$-Skelett von $S^n$ (das ist $\{ * \}$) abbildet.
...
So sind die stabilen Homotopiegruppen $\pi_n^s$ berechnet nur für $n \leq 64$ und unbekannt für größere $n$ (vgl. \cite[384]{hatcher:at}).
...
\begin{lem}
  Es sei $p : E \to B$ eine stetige Abbildung. Dann sind äquivalent:
...
\begin{proof}
  ...
  Es sei nun $(X, A)$ ein beliebiges Raumpaar.
  % TODO: Ausführlicher?
...
\begin{lem}
  Es sei $p : E \to B$ eine Serre-Faserung, $b_0 \in B$, $F \coloneqq p^{-1}(b_0)$ die Faser über $b_0$ und $f_0 \in F$.
...
\begin{defn}
  Es seien $p : E \to B$ und $g : X \to B$ stetig.
...
\begin{lem}
  Die langen exakten Sequenzen der Homotopiegruppen von Faserungen sind natürlich:
  Es sei $(g, \tilde{g}) : p' \to p$ ein Morphismus von Serre-Faserungen $p' : E' \to B'$ und $p : E \to B$, $b_0' \in B'$, $b_0 \coloneqq g(b_0')$, $F' \coloneqq p'^{-1}(b_0')$, $F \coloneqq p^{-1}(b_0)$, $f'_0 \in F'$, $f_0 \coloneqq \tilde{g}(f'_0)$. Dann gibt es eine "`Leiter"' bestehend aus kommutativen Quadraten zwischen den Homotopiesequenzen:
...
Es sei $p : E \to B$ eine Serre-Faserung, $\gamma : \I \to B$ ein stetiger Weg.
...
\begin{bem}
  \begin{itemize}
    ...
    \item Die Gesamtheit der Moduln $E^r_{p,q}$ und Differentiale $d^r_{pq}$ mit $r \in \N$ fest heißt \emph{$r$-te Seite} $E^r$.
...
\begin{defn}
  ...
  Der Grenzwert der Spektralsequenz ist die Unendlich-Seite $E^\infty \coloneqq \{ E^\infty_{p,q} \}_{p,q}$.
\end{defn}
...
\begin{defn}
  ...
  \[ 0 \subseteq \ldots \subseteq F^{p-1} M \subseteq F^p M \subseteq F^{p+1} M \subseteq \ldots \subset M \]
...
Man kann auch partielle Information über die $E^\infty$-Seite verwenden, um Aussagen über Differentiale zu treffen, z.\,B. dass ein Differential ein Isomorphismus ist.
...
\begin{satz}[Jean-Pierre Serre]
  Es sei $G$ eine abelsche Gruppe und $p : E \to B$ eine Serre-Faserung.
  Angenommen, $B$ ist wegzusammenhängend und $\pi_1(B)$ wirkt trivial auf $H_*(F_p)$.
...
Ein Beweis findet sich in \cite[Abschnitt 1.1]{hatcher:ss}.

Im Falle einer nichttrivialen Wirkung der Fundamentalgruppe existiert ebenfalls eine Serre-Spektralsequenz. Die Einträge auf der $E^2$-Seite haben dann die Form $E^2_{p,q} = H_p(B; \LH_q(F; G))$.
Dabei ist $\LH_q(F; G)$ keine feste Gruppe, sondern ein sogenanntes \emph{lokales Koeffizientensystem}, was bedeutet, dass die Koeffizienten vom Punkt $b \in B$ abhängen können.
...
Dann ist $H_p(B; \LH_q(F; G)) \cong H_p(B; \LH_q(F; G))$.
Im Falle einer nichttrivialen Wirkung von $\pi_1(B)$ gibt es geschlossene Wege $\gamma$, für die $\gamma_* : \LH_q(F; G)_b \to \LH_q(F; G)_b$ nicht die Identität ist.
...
Es sei $X$ ein zusammenhängender CW-Komplex.
Wähle Erzeuger $\Set{\phi_\alpha : S^n \to X}{\alpha \in A}$ von $\pi_n(X)$.
Benutze diese Abbildungen, um einen neuen CW-Komplex $X'$ auch $X$ durch Ankleben von $n{+}1$-Zellen $e_\alpha^{n+1}$ zu konstruieren.
Das heißt folgendes Diagramm ist ein Pushout:

\begin{centertikzcd}
  S^n \times A \arrow[d, swap, "i \times \id_A"] \arrow[r, "\amalg \, \phi_\alpha"] &
  X \arrow[d] \\
  D^{n+1} \times A \arrow[r] &
  X' \arrow[ul, phantom, "\lrcorner", very near start]
\end{centertikzcd}

\begin{lem}
  Die Abbildungen $\pi_j(X \xrightarrow{i} X')$ sind Isomorphismen für $j < n$ und $\pi_n(X') = 0$.
\end{lem}
...
\begin{proof}
  ...
  \end{itemize}
  Somit ist $\pi_j(i)$ bijektiv für $j < n$ und surjektiv für $j = n$.
  Um zu sehen, dass $\pi_n(X') = 0$, reicht es zu zeigen, dass $\pi_n(i)[f_\alpha] = [i \circ f_\alpha] = 0$ für alle $\alpha \in A$.
  Das gilt nach Konstruktion, da die charakteristische Abbildung $\Phi : e^{n+1}_\alpha \to X'$ eine Fortsetzung von $i \circ f_\alpha$ auf $D^{n+1}$ ist.
\end{proof}

Wir haben jedoch keine Kontrolle über die höheren Homotopiegruppen $\pi_j(X)$, $j > n$.
Diese können von der Wahl der Erzeuger abhängen.
Wir können jedoch mit dem Verfahren fortfahren, und diese Gruppen ebenfalls zu Null machen.
...
\begin{itemize}
  \item $X_m$ ensteht aus $X_{m-1}$ durch Ankleben von $(m{+}1)$-Zellen.
  \item $\pi_j(X_m \hookrightarrow X_k)$ ist ein Isomorphismus für $j \leq m$.
  \item $\pi_j(X_m) = 0$ für $n \leq j \leq m$
\end{itemize}
Wir setzen $\tau_{< n} \coloneqq \bigcup_{j=n}^\infty X_j$.

\begin{lem}
  Die Inklusion $i : X \to \tau_{< n} X$ induziert Isomorphismen $\pi_j(X) \to \pi_j(\tau_{< n} X)$ für $j < n$.
  Es gilt $\pi_j(\tau_{< n} X) = 0$ für $j \geq n$.
\end{lem}

\begin{proof}
  Aus zellulärer Approximation folgt:
  \begin{itemize}
    \item Jedes Element $x \in \pi_j(\tau_{< n} X)$ wird repräsentiert durch eine Abbildung $f : S^j \to X_j$.
...
Der Raum $\tau_{< n} X$ hängt auch nicht (bis auf Homotopie) von den Wahlen der Erzeuger ab:
Es seien dazu $Y, Y' \supset X$ zwei durch obiges Verfahren aus $X$ konstruierte Räume.
Dann kann man die Inklusionsabbildung $i : X \hookrightarrow Y'$ fortsetzen zu einer Abbildung $f : Y \to Y'$, indem man induktiv $f$ auf das $j$-Skelett, $j > n$ fortsetzt.
Dies ist möglich, da für eine $j$-Zelle $e_\beta^j$ die Einschränkung $f|_{\partial e_\beta^j}$ nullhomotop ist.
Die Abbildung $f$ ist dann eine Homotopieäquivalenz nach dem Whitehead-Theorem.

Nun können wir mit Lemma \ref{convert-to-fibration} die Abbildung $i : X \to \tau_{< n} X$ in eine Faserung $F \to E_i \to \tau_{< n} X$ mit $E_i \simeq X$ umwandeln.
Anhand der langen exakten Sequenz von Homotopiegruppen sieht man, dass $\pi_j(F \to E_i)$ ein Isomorphismus ist für $j \geq n$ und dass $\pi_j(F) = 0$ für $j < n$.
Wir haben es also geschafft, einen $(n{-}1)$-zusammenhängenden Raum $\tau_{\geq n} X \coloneqq F$ zu konstruieren, der im Grad $j \geq n$ diesselben Homotopiegruppen wie $X$ besitzt.
Mit anderen Worten: Wir haben die Homtopiegruppen unterhalb Grad $n$ getötet.
Die Faserung $\tau_{\geq n} X \to X' \to \tau_{< n} X$ mit $X' \coloneqq E_i \simeq X$ wird uns im Folgenden erlauben, $X$ zu untersuchen, indem wir $\tau_{\geq n} X$ und $\tau_{< n} X$ studieren.
Wir werden insbesondere die Faserung $\tau_{\geq n+1} S^n \to S^n \to \tau_{< n+1} S^n$ verwenden, um die Homotopiegruppen von $S^n$ zu untersuchen.
Der Raum $\tau_{< n+1} S^n$ besitzt nur eine nicht verschwindende Homotopiegruppe $\pi_n(\tau_{< n+1} S^n) \cong \Z$.
...
Mit anderen Worten ist $\tau_{< n+1} S^n$ also ein $K(\Z, n)$.  Man kann zeigen (siehe \cite[365-366]{hatcher:at}):

\begin{satz}
  Sei $n \geq 1$ und $G$ eine Gruppe, abelsch für $n \geq 2$.
  Dann existiert ein CW-Komplex $(X, x_0)$ vom Typ $K(G, n)$.
  Der CW-Komplex $X$ ist eindeutig bis auf Homotopieaquivalenz: Ist $X'$ ein weiterer CW-Komplex vom Typ $K(G, n)$, so gilt $X \simeq X'$.
\end{satz}
...
\begin{lem}
  Folgende Klassen erfüllen die Definition~\ref{serre-class}:
  \begin{enumerate}[label=\alph*)]
...
\begin{proof}
  \emph{a)} \enspace Es sei $0 \xrightarrow{f} A \to B \xrightarrow{g} C \to 0$ eine kurze exakte Squenz von abelschen Gruppen.
  ...
  Dann ist Rechtsmultiplikation mit $a^{-1}$ ein Isomorphismus $x \mapsto x \cdot a^{-1} : g^{-1}(c) \to g^{-1}(0)$ eine Bijektion mit Umkehrabbildung $x \mapsto x \cdot a$.
  Somit gilt $\abs{g^{-1}(c)} = \abs{g^{-1}(0)} = \abs{A}$.
  Für $n, m > 0$ gilt $\Z_n \otimes \Z_m \cong \Z_d$ und $\Tor(\Z_n, \Z_m) \cong \Z_d$ mit $d \coloneqq \ggT(n, m)$.
  Es seien nun $A = \Z_{n_1} \oplus \cdots \oplus \Z_{n_N}$, $B = \Z_{m_1} \oplus \cdots \oplus \Z_{m_M}$.
  Dann ist $\Tor(A, B) \cong A \otimes B \cong \bigoplus_{i=1}^N \bigoplus_{j=1}^M \Z_{\ggT(n_i, m_j)}$, also
  \[ \abs{\Tor(A, B)} = \abs{A \otimes B} = \prod_{i=1}^N \prod_{j=1}^M \ggT(n_i, m_j) \leq \min(\abs{B}^N, \abs{A}^M). \]

  \emph{b)} \enspace Wir betrachten nun wieder die kurze exakten Sequenz $0 \to A \to B \to C \to 0$ von abelschen Gruppen.
  Falls $B$ endlich erzeugt ist, so ist $C$ offensichtlich endlich erzeugt und $A$ ist endlich erzeugt als Untergruppe einer endlich erzeugten abelschen Gruppe.
  ...
  Schreibe $g(b) = m_1 \cdot c_1 + \ldots + m_M \cdot c_M$ mit $c_1, \ldots c_M \in \Z$.
  Dann ist $\tilde{b} \coloneqq b - (m_1 \cdot b_1 + \ldots + m_M \cdot b_M) \in \ker(g)$, \dh{} es gibt $n_1, \cdot, n_N \in \Z$ mit $\tilde{b} = n_1 \cdot f(a_1) + \cdots + n_N \cdot f(a_N)$.
  Somit $b = m_1 \cdot c_1 + \ldots + m_M c_M + n_1 \cdot a_1 + \ldots n_N a_N$.
  Für endlich erzeugte abelsche Gruppen $A$ und $B$ gibt es $n, n_1, \ldots, n_N \in \N$ und $m, m_1, \ldots, m_M \in \N$ mit $A \cong \Z^n \oplus \Z_{n_1} \oplus \ldots \oplus \Z_{n_N}$ und $B \cong \Z^m \oplus \Z_{m_1} \oplus \ldots \oplus \Z_{m_M}$.
...
\begin{proof}
  ...
  \[ E^2_{pq} \cong H_p(B; H_q(F; \Z)) \cong \left( H_p(B; \Z) \otimes H_q(F; \Z) \right) \oplus \Tor(H_{p-1}(B; \Z), H_q(F; \Z)). \]
...
Setze $X_n \coloneqq \tau_{< n+1} X$.
Dabei hat man $\tau_{< n+1} X$ aus $X$ durch Ankleben von Zellen der Dimension $\geq n+2$ konstruiert, sodass die Homotopiegruppen in Grad $\geq n+1$ verschwinden.
Die Abbildung $i : X \to X_n$ ist die Inklusion.
Diese lässt sich nun fortsetzen zu einer Abbildung $f : X_{n+1} \to X_n$:
Sei dazu induktiv eine Fortsetzung $f^k : X_{n+1}^k \cup X \to X_n$, $k \geq n+1$ von $i$ gegeben, wobei $X_{n+1}^k$ das $k$-Skelett von $X_{n+1}$ bezeichnet.
Dann lässt sich die Abbildung auch auf $(k+1)$-Zellen $e^{k+1}_\alpha$ ausdehnen, denn $f^k|_{\partial \partial e^{k+1}_\alpha}$ ist nullhomotop, da $\pi_k(X_n) = 0$ für $k \geq n+1$.
