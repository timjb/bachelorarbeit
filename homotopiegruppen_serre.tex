...
\usetikzlibrary{cd}
\usepackage{geometry}
\geometry{left=2.65cm,right=2.65cm,top=2cm,bottom=2cm}
\usepackage{xfrac}
...
\newcommand{\homeo}{\approx} % homöomorph
...
\]
modulo Homotopie durch denselben Typ von Abbildung definieren. % Wir werden beide Sichtweisen in dieser Arbeit gebrauchen.
...
%Dafür untersuchen wir die $p$-Torsion in der Homologie von $K(\Z, n)$.
...
\begin{defn}\label{def:fibration}
  ...
  Der Raum $E$ wird \emph{Totalraum} und $B$ wird \emph{Basisraum} genannt.
\end{defn}

\begin{bem}
  Allgemeiner ist eine \emph{Serre-Faserung} eine stetige Abbildung $p : E \to B$, welche die Homotopieliftungseigenschaft bezüglich der Scheiben $\Set{D^n}{n \geq 0}$ oder äquivalent allen CW-Komplexen, aber nicht unbedingt allen topologischen Räume besitzt.
...
\begin{bsp}
  Das einfachste Beispiel einer Faserung ist die \emph{Produktfaserung}, die Projektion $p_1 : B \times F \to B$ eines Produktraumes auf einen Faktor.
  ...
  \[
    ...
    (x, t) \mapsto (H(x, t), p_2(H_0(x))).
  \]
\end{bsp}

Bei dieser Produktfaserung sind die \emph{Fasern}, das heißt die Urbilder $\Set{p^{-1}(\{b\})}{b \in B}$ von Einpunktmengen versehen mit der Teilraumtopologie, alle homöomorph zu dem festen Raum $F$.
Allgemein sind bei Faserungen über wegzusammenhängenden Basisräumen alle Fasern zwar nicht immer homöomorph, aber homotopieäquivalent:

\begin{lem}[{\cite[Prop 4.61]{hatcher:at}}]
  Es sei $p : E \to B$ eine Faserung und $\gamma : I \to B$ ein stetiger Weg von $a \coloneqq \gamma(0)$ nach $b \coloneqq \gamma(1)$.
  Wegen der Homotopieliftungseigenschaft gibt es ein $\widetilde{H}_\gamma$, sodass
  \begin{centertikzcd}[row sep=1.2cm, column sep=1.6cm]
...
\begin{proof}
  Wir halten zunächst fest, dass jede Faserung $p$ auch folgende relative Homotopieliftungseigenschaft erfüllt: Sei das äußere kommutative Quadrat im Diagramm
  \begin{centertikzcd}[row sep=1.2cm, column sep=1.6cm]
  ...
  Wir benutzen dann die gewöhnliche Homotopieliftungseigenschaft mit $H' \coloneqq H \circ (\id_X \times \psi)$ und $H_0' \coloneqq H_0 \circ (\id_X \times \psi|_{\I \times \{0\}})$ und erhalten einen Lift $\tilde{H}'$.
  Dann kann man leicht nachrechnen, dass $\tilde{H} \coloneqq \tilde{H}' \circ (\id_X \times \psi^{-1})$ ein geeigner Lift im obigen Diagramm ist.
  ...
  \begin{align*}
    ...
    & H_0(f, t, 0) \coloneqq f, \quad
    H_0(f, 0, s) \coloneqq \tilde{H}_\gamma(f, s), \quad
  ...
  $\tilde{H}(\blank, \blank, 1) : p^{-1}(\{a\}) \times \I \to p^{-1}(\{b\})$.
...
\begin{proof}
  ...
  \[ \ldots \to \pi_n(F, f_0) \xrightarrow{i_*} \pi_n(E, f_0) \xrightarrow{j_*} \pi_n(E, F, f_0) \xrightarrow{\partial} \pi_{n-1}(F, f_0) \to \ldots \to \pi_1(E, F, f_0) \]
  des Raumpaares $(E, F)$.
  ...
  \[ p_* = \left( \pi_n(E, f_0) \xrightarrow{j_*} \pi_n(E, F, f_0) \xrightarrow{\cong} \pi_n(B, b_0) \right). \]
  Wir zeigen: $p_* : \pi_n(E, F, f_0) \to \pi_n(B, b_0)$ ist der gesuchte Isomorphismus (damit ist obige Gleichung erfüllt).

  \emph{Surjektivität}: Sei $[g : (I^{n+1}, \partial I^{n+1}, *) \to (B, \{ b_0 \}, b_0)] \in \pi_{n+1}(B, b_0)$, $n \geq 0$.
  Sei $\tilde{g}$ der Lift im relativen HLE-Diagramm
  \begin{centertikzcd}[row sep=1.2cm, column sep=1.4cm]
    ...
    B
  \end{centertikzcd}
  % XXX: Diese Hochhebungseigenschaft irgendwo zeigen
  ...
  Es gilt $p_*[\tilde{g}] = [p \circ \tilde{g}] = [g]$.

  \emph{Injektivität}: Seien $[h_0], [h_1] \in \pi_{n+1}(E, F, f_0)$ mit $p_*[h_0] = p_*[h_1]$.
  Sei
  \[
    H : (I \times I^{n+1}, I \times \partial I^n) \to (B, \{ b_0 \}), \quad
    (t, x) \mapsto H_t(x)
  \]
  ...
  \begin{centertikzcd}[row sep=1.2cm, column sep=1.8cm]
    ...
    B
  \end{centertikzcd}
  % XXX: Vereinfachen mit alternativer Definition der Homotopiegruppen
  mit $V \coloneqq (I^{n+1} \!\times\! \{ 0 \}) \cup (\partial I^{n+1} \!\times\! I) = (\{ 0, 1 \} \times I^{n+1}) \cup I \times U \subset I^{n+2}$ und
  \[
    h|_{\{0\} \times I^{n+1}} \coloneqq h_0, \quad
    h|_{\{1\} \times I^{n+1}} \coloneqq h_1, \quad
    h|_{I \times U} \coloneqq \const{f_0}.
  \]
  Nun ist $\tilde{H}$ eine Homotopie von $h_0$ nach $h_1$, welche zu jedem Zeitpunkt $t$ eine Abbildung $\tilde{H}_t : (I^{n+1}, \partial I^{n+1}, U) \to (E, F, \{ b_0 \}), \enspace x \mapsto \tilde{H}(t, x)$ von Raumtripeln ist.
\end{proof}
...
\begin{bsp}
  ...
  \[ \eta^{-1}(c) = \Set{\nicefrac{s}{\norm{(c,1)^T}} \cdot (c, 1)^T}{s \in \C, \abs{s} = 1} \homeo S^1 \]
  für alle $c \in \C$ ist.
...
Wir benutzen diese Abbildungen, um einen neuen CW-Komplex $X'$ aus $X$ durch Ankleben von $n{+}1$-Zellen $e_\alpha^{n+1}$ zu konstruieren.
...
\begin{proof}
  Aus zellulärer Approximation (siehe \cite[Thm 4.8]{hatcher:at}) folgt:
  \begin{itemize}
...
\begin{proof}
  ...
  Zunächst gilt $H_n(X) \in \SC$ genau dann, wenn die Gruppen $E^\infty_{i,n-i}$ für $i = 0, \ldots, n$ alle in $\SC$ liegen, denn diese Gruppen sind die Quotienten einer endlichen Filtrierung von $H_n(X)$.
  ...
  Da aber $E^{k+1}_{k,0} \cong E^\infty_{k,0} \in \SC$, gilt $E^r_{k,0} \in \SC$ für alle $r \geq 2$.
  ...
  Dazu verwendet man die kurze exakte Sequenz $0 \to \im(d^r_{r,k-r+1}) \to E^r_{0,k} \to E^{r+1}_{0,k} \to 0$.
\end{proof}
...
\begin{proof}
  ...
  \begin{centertikzcd}
    ...
    H_n(X_n)
  \end{centertikzcd}
  Im Beweis des letzten Lemmas wurde gezeigt, dass der untere Morphismus ein Isomorphismus ist.
  Es genügt daher zu zeigen, dass $h_n : \pi_n(X_n) \to H_n(X_n; \Z)$ ein Isomorphismus modulo $\SC$ ist.
  Wir betrachten die Serre-Spektralsequenz zur Faserung $F_n \to X_n \to X_{n-1}$, wobei $F_n$ ein $K(\pi_n(X), n)$ ist.
  Es gilt
  \[
    E^2_{p,q} \cong
  ...
  \]
  \dh{} es verschwinden also alle Einträge zwischen der $0$-ten und der $n$-ten Zeile.
  ...
  Somit ist der mittlere Morphismus $H_n(F_n) \to H_n(X_n)$ ein Isomorphismus modulo~$\SC$.
  Dieser Morphismus ist aber gerade der von der Inklusion $F_n \hookrightarrow X_n$ induzierte Morphismus in Homologie.\footnote{
    Dies sieht man durch nähere Betrachtung des Beweises über die Existenz der Serre-Spektralsequenz zu einer Faserung $F \to X \to B$ in \cite{hatcher:ss}:
    Durch CW-Approximation kann man davon ausgehen, dass $B$ ein CW-Komplex ist.
    Die Filtrierung von $H_n(X)$ wird dort definiert durch $F^p H_n(X) \cong \im(H_n(p^{-1}(B^p) \hookrightarrow X)) \subseteq H_n(X)$, wobei~$B^p$ das $p$-Skelett von $B$ bezeichnet.
    Die Abbildung $H_n(F) \cong H_0(B; H_n(F)) \cong E^2_{0,n} \to E^\infty_{0,n} \cong F^0 H_n(X)$ wird durch die Inklusion von $F \homeo p^{-1}(B^0)$ in $X$ induziert.
  }

  Wir betrachten nun das kommutative Diagramm
  \begin{centertikzcd}
...
\begin{proof}
  ...
  % XXX: kommutatives Diagramm dazu?
  Es gilt
...
\begin{proof}
  ...
  \[
    ...
    \pi_i(S^n)
    \enspace\to\enspace
    \underbrace{\pi_i(M_i)}_{\mathclap{\cong \pi_j(K(\Z, n))}}
    \enspace\to\enspace
    \pi_i(M_i, S^n)
    \enspace\to\enspace
    \pi_{i-1}(S^n)
...
% TODO:
% * Relative Homotopie-Gruppen definieren
% * Verwendung von Hurewicz- und Serre-Faserungen überprüfen
% * Schon früher den Beginn des Faserbündels einführen (oder vllt. sogar gar nicht)
% * Schreiben, was jetzt genau ich gemacht habe
