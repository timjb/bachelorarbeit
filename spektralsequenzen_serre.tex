...
\begin{defn}
  ...
  Der Grenzwert der SS ist die Unendlich-Seite $E^\infty \coloneqq \{ E_\infty^{p,q} \}_{p,q}$.
\end{defn}
...
\begin{defn}
  Eine Spektralsequenz $E$ \emph{konvergiert gegen} einen graduierten $A$-Modul $M = \oplus_{n \in \Z} M_n$, falls $E$ überhaupt konvergiert und Filtrierungen $\ldots \subseteq F_p M_n \subseteq F_{p+1} M_n \subseteq \ldots$ existieren, sodass für alle $p, q \in \Z$ der Eintrag $E_\infty^{p+q}$ isomorph zum Quotienten $F_{p+1} M_{p+q} / F_p M_{p+q}$ ist.
\end{defn}
