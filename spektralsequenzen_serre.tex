...
\begin{proof}
  ...
  Der Satz gilt für $n = 1$, denn die Sphäre $S^1$ ist ein $K(\Z, 1)$ und es gilt bekanntermaßen $H^*(S^1; R) \cong \Lambda_R[x]$ für $R = \Z$ und somit auch für $R = \Q$. \\
  Im Induktionsschritt nutzen wir die Pfadraumfaserung $K(\Z, n-1) \to P \to K(\Z, n)$. Da $K(\Z, n)$ für $n \geq 2$ einfach zusammenhängend ist, gilt für deren Serre-Spektralsequenz $E_2^{p,q} \cong H^p(K(\Z, n); H^q(K(\Z, n-1)))$. \\
  \emph{Falls $n$ gerade:} \enspace
  \begin{center}
    \begin{tikzpicture}[x=16pt,y=16pt]
      \begin{scope}[shift={(0,0)}]
