...
Die Faserung $\tau_{\geq n} X \to X' \to \tau_{< n} X$ mit $X' \coloneqq E_i \simeq X$ wird uns im Folgenden erlauben, (die Homotopiegruppen von) $X$ zu untersuchen, indem wir $\tau_{\geq n} X$ und $\tau_{< n} X$ studieren.
...
\begin{bspe}
  Man kann leicht zeigen, dass folgende Klassen die Definition~\ref{serre-class} erfüllen:
...
\begin{proof}
  Es sei $0 \to A \to B \to C \to 0$ eine kurze exakte Squenz von abelschen Gruppen.
  Falls $B$ endlich ist, so
\end{proof}
