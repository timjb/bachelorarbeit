...
\begin{defn}
  ...
  Für $n \geq 1$ heißt die Menge
  \[ \pi_n(X, x_0) \coloneqq [(S^n, *), (X, x_0)] \]
  der basispunkterhaltenden Abbildungen $S^n \to X$ modulo basispunkterhaltender Homotopie \emph{$n$-te Homotopiegruppe} von $X$ (mit Basispunkt $x_0$).
\end{defn}
...
\begin{defn}
  Die \emph{Einhängung} $\Sigma X$ eines punktierten Raumes $(X, x_0)$ ist
...
\begin{defn}
  Ein nicht leerer topologischer Raum $X$ heißt \emph{$n$-zusammenhängend}, falls $X$ wegzusammenhängend ist und $\pi_i(X) = 0$ für $1 \leq i \leq n$.
\end{defn}

% TODO: Hatcher zitieren, p. 360
\begin{satz}[Freudenthal'scher Einhängungssatz]
  Es sei $X$ ein $(n{-}1)$-zusammenhängender CW-Komplex.
  Dann ist $E : \pi_i(X) \to \pi_{i+1}(\Sigma X)$ ein Isomorphismus für $i < 2n - 1$ und ein Epimorphismus für $i = 2n$.
\end{satz}

% TODO: Stabile Homotopiegruppen erklären
