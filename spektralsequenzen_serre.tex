...
% TODO: Indizierung andersrum bei Kohomologie?
\begin{defn}
  Eine \emph{Filtrierung} eines $A$-Moduls $M$ ist eine aufsteigende Folge $\ldots \subseteq F_p M \subseteq F_{p+1} M \subseteq \ldots$ von Untermoduln von $M$, $p \!\in\! \Z$.
...
\begin{defn}
  Eine Spektralsequenz $E$ \emph{konvergiert gegen} einen graduierten $A$-Modul $M = \oplus_{n \in \Z} M_n$ (notiert $E_r^{p,q} \Rightarrow M_{p+q}$), falls $E$ überhaupt konvergiert und Filtrierungen $\ldots \subseteq F_p M_n \subseteq F_{p+1} M_n \subseteq \ldots$ existieren, sodass für alle $p, q \in \Z$ der Eintrag $E_\infty^{p+q}$ isomorph zum Quotienten $F_{p+1} M_{p+q} / F_p M_{p+q}$ ist.
\end{defn}
...
\begin{defn}
  Eine \emph{Filtrierung eines Kokettenkomplexes} $C^\bullet$ ist eine aufsteigende Folge von Unterkomplexen $\ldots \subseteq F^p C^\bullet \subseteq F^{p+1} C^\bullet \subseteq \ldots \subseteq C^\bullet$.
\end{defn}
...
\begin{lem}
  ...
  $E_2^{pq} = H^q(F^p C^\bullet / F^{p-1} C^\bullet)$.
  % TODO: In welchem Sinne?
  Falls die Filtrierung in einem gewissen Sinne endlich ist, so konvergiert diese Spektralsequenz gegen $H^*(C^\bullet)$.
\end{lem}
...
\begin{lem}
  \ES{} $z_1 \leq q_1 \leq p_1 = z_2 \leq b_1 = q_2 \leq p_2 \leq b_2$.
...
\begin{lem}
  ...
  \begin{centertikzcd}[row sep=0.2cm, column sep=0.5cm]
    &&&& z_3 \arrow[r, "\leq" description, phantom] \arrow[d, equal] &
    q_3 \arrow[r, "\leq" description, phantom] \arrow[d, equal] &
    p_3 \arrow[r, "\leq" description, phantom] &
    b_3 \\

    && z_2 \arrow[r, "\leq" description, phantom] \arrow[d, equal] &
    q_2 \arrow[r, "\leq" description, phantom] \arrow[d, equal] &
    p_2 \arrow[r, "\leq" description, phantom] &
    b_2 \\

    z_1 \arrow[r, "\leq" description, phantom] &
    q_1 \arrow[r, "\leq" description, phantom] &
    p_1 \arrow[r, "\leq" description, phantom] &
    b_1
  \end{centertikzcd}
  Dann ist
  \[
...
\begin{proof}[Beweis des Lemma über Spektralsequenz]
  TODO
\end{proof}
